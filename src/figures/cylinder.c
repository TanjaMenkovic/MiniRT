...
float hit_bot_cap(t_vector axis_dir, t_vector bot_cap_center, t_ray ray, float radius)
{
    float t_cap_bottom;
    t_vector point_bottom;
    t_vector vec_bottom;

    t_cap_bottom = hit_plane(axis_dir, bot_cap_center, ray);
    if (t_cap_bottom > 0.0f) {
        point_bottom = vec_add(ray.start, vec_mult(ray.direction, t_cap_bottom));
        vec_bottom = vec_sub(point_bottom, bot_cap_center);
        if (dot_prod(vec_bottom, vec_bottom) > radius * radius)
            t_cap_bottom = -1.0f;
    }
    return (t_cap_bottom);
...
float hit_cyl_body(t_ray ray, t_vector base_center, t_vector axis_dir, float radius, float height)
{
    ...
    cy.z1 = dot_prod(vec_sub(cy.p1, base_center), axis_dir);
    cy.z2 = dot_prod(vec_sub(cy.p2, base_center), axis_dir);
    if (cy.t1 > 0 && cy.z1 >= -height / 2.0f && cy.z1 <= height / 2.0f)
        cy.t_cylinder = cy.t1;
    else if (cy.t2 > 0 && cy.z2 >= -height / 2.0f && cy.z2 <= height / 2.0f)
        cy.t_cylinder = cy.t2;
    return (cy.t_cylinder);
}

float hit_cylinder(t_vector center, t_vector axis_dir, float radius, float height, t_ray ray)
{
    float t_cylinder;
    float t_cap_top;
    float t_cap_bottom;
    t_vector top_cap_center;
    t_vector bot_cap_center;

    axis_dir = unit_vector(axis_dir);

    // Calculate base and top centers
    bot_cap_center = vec_sub(center, vec_mult(axis_dir, height / 2.0f));
    top_cap_center = vec_add(center, vec_mult(axis_dir, height / 2.0f));

    t_cylinder = hit_cyl_body(ray, center, axis_dir, radius, height);
    t_cap_top = hit_top_cap(axis_dir, top_cap_center, ray, radius);
    t_cap_bottom = hit_bot_cap(axis_dir, bot_cap_center, ray, radius);
...
t_vector cyl_normal(t_vector point, t_vector center, t_vector axis_dir, float height, float radius)
{
    float   proj_length;
    t_vector p_minus_c;
    t_vector proj_p_minus_c;
    ...
    // Calculate base and top centers
    t_vector base_center = vec_sub(center, vec_mult(axis_dir, height / 2.0f));
    t_vector top_center = vec_add(center, vec_mult(axis_dir, height / 2.0f));

    p_minus_c = vec_sub(point, center);
    proj_length = dot_prod(p_minus_c, axis_dir);

    if (proj_length >= height / 2.0f)
        return axis_dir;
    else if (proj_length <= -height / 2.0f)
        return vec_mult(axis_dir, -1.0f);
